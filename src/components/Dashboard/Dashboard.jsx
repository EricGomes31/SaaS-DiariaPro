...
import { ArrowDownRight, ArrowUpRight, CalendarDays, ChevronRight, DollarSign, MapPin, TrendingUp, Users } from 'lucide-react'
        ...
        <div style={{ flex: 1 }}>
          ...
          {change != null && (
            <div
              ...
              {change < 0 ? <ArrowDownRight size={13} color="#f43f5e" /> : <ArrowUpRight size={13} color="#10b981" />}
              <span
                style={{
                  fontSize: 11,
                  color: change < 0 ? '#f43f5e' : '#10b981',
                  fontWeight: 600,
                }}>
                {change >= 0 ? '+' : ''}
                {change}%
...
export default function Dashboard({ lang = 'pt', onNavigate, workers, workDays, locations, currentUser }) {
  ...
  const avgChange = pctChange(curMonthDays.length ? sumEarnings(curMonthDays) / curMonthDays.length : 0, prevMonthDays.length ? sumEarnings(prevMonthDays) / prevMonthDays.length : 0)
  ...
  const greetingWord = lang === 'en' ? (hour < 12 ? 'Good morning' : hour < 18 ? 'Good afternoon' : 'Good evening') : lang === 'es' ? (hour < 12 ? 'Buenos días' : hour < 18 ? 'Buenas tardes' : 'Buenas noches') : hour < 12 ? 'Bom dia' : hour < 18 ? 'Boa tarde' : 'Boa noite'

  return (
    <div>
      {/* Page header */}
      <motion.div initial={{ opacity: 0, y: -10 }} animate={{ opacity: 1, y: 0 }} transition={{ duration: 0.4 }} style={{ marginBottom: isMobile ? 20 : 36 }}>
        <div
          style={{
            fontSize: 13,
                ...
                content={({ active, payload }) => {
                  ...
                  return (
                    <div
                      ...
                      <div style={{ fontSize: 13, color: 'var(--card-heading)' }}>
                        {d.days} {t.days}
                      </div>
                    </div>
                  )
