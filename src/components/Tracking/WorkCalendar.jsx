...
import { ChevronLeft, ChevronRight, Plus, X, MapPin, Users, Clock, Check, AlertCircle } from 'lucide-react'
...
export default function WorkCalendar({ lang = 'pt', workers, workDays, setWorkDays, locations, holidays = [], paymentRecords = [] }) {
  ...
  const handleSaveOvertime = (entry) => {
    ...
    const updated = { ...entry, overtime: amount, earnings: entry.rate + amount + (entry.bonus || 0) }
  ...
  const filteredDays = useMemo(() => workDays.filter(d => {
  ...
  const workerHistory = useMemo(
    () => filterWorkerObj ? [...filteredDays].sort((a, b) => b.date.localeCompare(a.date)) : [],
    [filteredDays, filterWorkerObj],
  )
  ...
  return (
    <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
      ...
      <div style={{ display: 'flex', flexDirection: 'column' }}>
          ...
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: isMobile ? 'flex-start' : 'flex-end', flexWrap: 'wrap', gap: isMobile ? 12 : 0, marginBottom: 20 }}>
            ...
            <div style={{ display: 'flex', gap: 10, flexWrap: 'wrap', width: isMobile ? '100%' : 'auto' }}>
              ...
              <SearchableSelect
                ...
                options={[
                  { value: 'all', label: t.allWorkers },
                  ...workers
                    .filter(w => filterDept === 'all' || w.department === filterDept)
                    .map(w => ({ value: w.id, label: w.name })),
                ]}
                placeholder={t.allWorkers}
        ...
        >
          {/* Day headers */}
          <div translate="no" style={{ display: 'grid', gridTemplateColumns: `repeat(7, 1fr) ${isMobile ? 40 : 60}px`, gap: 4, marginBottom: 8 }}>
          ...
          <div style={{ display: 'grid', gridTemplateColumns: `repeat(7, 1fr) ${isMobile ? 40 : 60}px`, gap: 4 }}>
            {(() => {
              ...
              return (
                <motion.div
                  ...
                  whileTap={{ scale: 0.96 }}
                  onClick={() => { setSelectedDay(prev => prev && isSameDay(prev, day) ? null : day); setAddError('') }}
                  style={{
                ...
                >
                  {allPaid ? (
                    <div title={t.statusPaid} style={{ position: 'absolute', top: 4, right: 4, width: 14, height: 14, borderRadius: '50%', background: '#10b981', display: 'flex', alignItems: 'center', justifyContent: 'center', boxShadow: '0 1px 4px rgba(16,185,129,0.5)' }}>
                      <Check size={9} color="#fff" strokeWidth={3} />
                    </div>
                  ) : somePaid ? (
                    <div title={t.statusPaid} style={{ position: 'absolute', top: 5, right: 5, width: 7, height: 7, borderRadius: '50%', background: '#10b981', opacity: 0.65 }} />
                  ) : todayDay && !isSelected ? (
                  ...
                  {entries.length > 0 && (
                    <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                      ...
                      <div style={{ display: 'flex', alignItems: 'center', gap: 3, fontSize: 10, color: isSelected ? '#818cf8' : 'var(--card-muted)', fontWeight: 700 }}>
                    ...
                    <div style={{
                      ...
                      {weekCount > 0 && (
                        <>
                          ...
                          <span style={{ fontSize: isMobile ? 11 : 13, fontWeight: 800, color: '#818cf8' }}>{weekCount}</span>
                        </>
                      )}
                    </div>
                  </Fragment>
          ...
          {selectedDay ? (
            ...
            >
              ...
              <AnimatePresence>
                {showAddPanel && (
                  ...
                  >
                    <div style={{ padding: '16px', borderRadius: 14, background: 'rgba(99,102,241,0.06)', border: '1px solid rgba(99,102,241,0.15)', marginBottom: 16, maxWidth: 460 }}>
                          ...
                          {locations.map(l => {
                            ...
                            return (
                              <motion.button
                                ...
                                type="button"
                                whileTap={{ scale: 0.96 }}
                                onClick={() => setAddForm(p => ({ ...p, locationId: l.id }))}
                                style={{
                              ...
                              >
                                <div style={{ width: 7, height: 7, borderRadius: '50%', background: l.color, opacity: sel ? 1 : 0.4, flexShrink: 0 }} />
                                {l.name}
                              </motion.button>
                            )
                          })}
                      ...
                      <div style={{ display: 'flex', gap: 8 }}>
                        <button
                          onClick={() => { setShowAddPanel(false); setWorkerSearch(''); setAddError('') }}
                          style={{
                      ...
                      >
                        ...
                        <div style={{ display: 'flex', alignItems: 'center', gap: 8, marginBottom: 8 }}>
                          ...
                          <div style={{ flex: 1, minWidth: 0, paddingRight: 18 }}>
                            <div style={{ fontSize: 13, fontWeight: 600, color: 'var(--card-heading)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }} title={worker?.name}>{worker?.name}</div>
                        ...
                        {overtimeEntry === entry.id ? (
                          <div style={{ display: 'flex', flexDirection: 'column', gap: 4 }}>
                            <div style={{ display: 'grid', gridTemplateColumns: 'auto 1fr auto auto', gap: 4, alignItems: 'center' }}>
                              <span style={{ fontSize: 10, color: 'var(--card-muted)', whiteSpace: 'nowrap' }}>R$:</span>
                              <input
                                type="number"
                                min="0"
                                step="0.01"
                            ...
                            {parseFloat(overtimeHours) > 0 && (
                              <div style={{ fontSize: 10, color: '#10b981' }}>
                                + R$ {(parseFloat(overtimeHours) || 0).toFixed(2)}
                              </div>
                            )}
                          </div>
                        ) : (
                          <button
                            onClick={() => { setOvertimeEntry(entry.id); setOvertimeHours((entry.overtime || 0) > 0 ? String(entry.overtime) : '') }}
                            style={{
            ...
            >
              ...
              <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center', flexWrap: 'wrap', gap: 12, marginBottom: 20 }}>
                <div style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                  <div style={{
                    ...
                    background: `${filterWorkerObj.avatarColor}20`, border: `1.5px solid ${filterWorkerObj.avatarColor}30`,
                    display: 'flex', alignItems: 'center', justifyContent: 'center',
                    fontSize: 14, fontWeight: 700, color: filterWorkerObj.avatarColor, flexShrink: 0,
                  }}>
                    {filterWorkerObj.avatar}
                  </div>
                  <div>
                    <div style={{ fontSize: 11, color: 'var(--card-muted)', marginBottom: 2, textTransform: 'uppercase', letterSpacing: '0.08em' }}>
                      {t.workerHistoryTitle}
                    </div>
                    <div style={{ fontFamily: 'Syne, sans-serif', fontSize: 20, fontWeight: 800, color: 'var(--card-heading)', letterSpacing: '-0.02em' }}>
                ...
                <div style={{ display: 'flex', alignItems: 'center', gap: 10, flexWrap: 'wrap' }}>
                  <div style={{
                    ...
                    <span style={{ fontSize: 16, fontWeight: 800, color: '#818cf8' }}>{workerHistory.length}</span>
              ...
              {workerHistory.length > 0 ? (
                <div style={{
                  border: '1px solid var(--inner-border)', borderRadius: 14,
                  overflow: 'hidden', maxHeight: 460, overflowY: 'auto',
                }}>
                  {/* Table header */}
                  <div style={{
                    display: 'grid', gridTemplateColumns: '1.3fr 1fr auto', gap: 12,
                    padding: '10px 16px', position: 'sticky', top: 0, zIndex: 1,
                    background: 'var(--inner-bg)', borderBottom: '1px solid var(--inner-border)',
                    fontSize: 11, fontWeight: 600, color: 'var(--card-muted)',
                  ...
                  {workerHistory.map((entry, i) => {
                    ...
                    return (
                      <div
                        key={entry.id}
                        onClick={() => { setCurrentMonth(date); setSelectedDay(date) }}
                        style={{
                          display: 'grid', gridTemplateColumns: '1.3fr 1fr auto', gap: 12,
                        ...
                        onMouseLeave={e => e.currentTarget.style.background = 'transparent'}
                      >
                        <span style={{ fontSize: 13, fontWeight: 600, color: entry.isWeekend ? '#f59e0b' : 'var(--card-heading)', textTransform: 'capitalize' }}>
                        ...
                        <span style={{ display: 'flex', alignItems: 'center', gap: 6, minWidth: 0 }}>
                          ...
                          <span style={{ fontSize: 12, color: 'var(--card-sub)', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' }}>{loc?.name}</span>
                        </span>
                        <div style={{ textAlign: 'right', whiteSpace: 'nowrap' }}>
                          <span style={{ fontSize: 14, fontWeight: 800, color: entry.isWeekend ? '#f59e0b' : '#818cf8' }}>
                            R$ {entry.earnings}
                          </span>
                          {(entry.overtime || 0) > 0 && (
                          ...
                          {isPaid(entry) && (
                            <div style={{ fontSize: 10, color: '#10b981', fontWeight: 700, display: 'flex', alignItems: 'center', justifyContent: 'flex-end', gap: 3, marginTop: 2 }}>
