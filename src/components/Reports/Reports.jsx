...
import { BarChart3, CalendarDays, CheckCircle2, ChevronDown, Coffee, Cookie, Download, FileSpreadsheet, FileText, Filter, Loader2, Mail, PieChart as PieChartIcon, TrendingUp, Users, UtensilsCrossed, X } from 'lucide-react'
...
function exportCSV({ topEarners, byLocation, monthlyData, byDayOfWeek, period }) {
  ...
  const lines = [
    ...
    ...topEarners.map((w, i) => [`${i + 1}º`, w.name, w.jobTitle, w.department, w.totalEarnings.toFixed(2).replace('.', ',')].join(';')),
    '',
...
function ExportModal({ onClose, exportData, workers, workDays, locations, t }) {
  ...
  const FORMATS = [
    {
      id: 'csv',
      label: t.reportsCsvLabel,
      ext: '.csv',
      icon: FileSpreadsheet,
      color: '#10b981',
      desc: t.reportsCsvDesc,
    },
    {
      id: 'pdf',
      label: t.reportsPdfLabel,
      ext: '.pdf',
      icon: FileText,
      color: '#6366f1',
      desc: t.reportsPdfDesc,
    },
  ]
      ...
      <motion.div
        ...
        <div style={{ padding: '22px 24px' }}>
          ...
          <div
            ...
            {[
              {
                label: t.workersLabel ?? 'Diaristas',
              ...
              {
                label: 'Total (R$)',
                value: `R$ ${previewTotal.toLocaleString('pt-BR')}`,
                color: '#f59e0b',
              },
            ].map((s, i) => (
...
function computeExpenseStats(workDays, dailyExpenses, locale, dayNames) {
  ...
  const monthly = Array.from({ length: 6 }, (_, i) => {
    ...
    for (const date in byDate)
      if (date.slice(0, 7) === key) {
        breakfast += byDate[date].cafe
        lunch += byDate[date].almoco
        snack += byDate[date].lanche
      }
    return {
      label: cap(format(md, 'MMM', { locale }).replace('.', '')),
      breakfast,
      lunch,
      snack,
    }
  ...
  const weekday = (dayNames ?? ['Dom', 'Seg', 'Ter', 'Qua', 'Qui', 'Sex', 'Sáb']).map(name => ({ label: name, breakfast: 0, lunch: 0, snack: 0 }))
  for (const date in byDate) {
  ...
  const totals = Object.values(byDate).reduce(
    (a, e) => ({
      breakfast: a.breakfast + e.cafe,
      lunch: a.lunch + e.almoco,
      snack: a.snack + e.lanche,
          ...
          <div
            ...
            <motion.div
              ...
              {topEarners.map((w, i) => {
                ...
                return (
                  <motion.div key={w.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }} transition={{ delay: 0.25 + i * 0.05 }} style={{ marginBottom: i < 4 ? 16 : 0 }}>
                    <div
                      style={{
                        display: 'flex',
                        justifyContent: 'space-between',
                      ...
                      <div
                        ...
                        <span
                          style={{
                            fontSize: 13,
                            fontWeight: 700,
                            color: i === 0 ? '#f59e0b' : 'var(--card-muted)',
                            width: 16,
                          }}>
                          {i === 0 ? '🥇' : i === 1 ? '🥈' : i === 2 ? '🥉' : `${i + 1}.`}
                        </span>
                        <div
                          style={{
                            ...
                            border: `1.5px solid ${w.avatarColor}30`,
                            display: 'flex',
                            alignItems: 'center',
                            justifyContent: 'center',
                            fontSize: 10,
