...
import { AlertTriangle, CalendarDays, CheckCircle2, DollarSign, Download, FileSpreadsheet, Layers, RotateCcw, Upload, Users, X } from 'lucide-react'
...
const UI = {
  ...
  en: {
    ...
    steps: ['Choose above what to import (workers, work days or payments).', 'Download the spreadsheet template with the button below.', 'Fill one row per record. Fields marked as required cannot be empty.', 'Save as CSV and upload. Rows with empty required fields come back as a CSV to fix.'],
  ...
  es: {
    ...
    steps: ['Elige arriba qué importar (trabajadores, días o pagos).', 'Descarga la plantilla con el botón de abajo.', 'Completa una fila por registro. Los campos marcados como obligatorio no pueden quedar vacíos.', 'Guarda como CSV y súbelo. Las filas con campos obligatorios vacíos vuelven en un CSV para corregir.'],
...
const TYPES = {
  ...
  workDays: {
    ...
    build: (get, ctx, idx) => {
      ...
      const defRate = dow === 0 ? (worker.sundayRate ?? worker.saturdayRate ?? worker.weekdayRate) : dow === 6 ? (worker.saturdayRate ?? worker.weekdayRate) : worker.weekdayRate
...
const ALL_EXAMPLES = [
  {
    tipo: 'diarista',
    nome: 'JOÃO DA SILVA',
    tipo_trabalhador: 'diarista',
    departamento: 'Serra Park',
    cargo: 'Doca',
    diaria_semana: '170',
    diaria_sabado: '170',
    diaria_domingo: '180',
...
const SUBTYPE_BY_TIPO = {
  diarista: 'workers',
  dia: 'workDays',
  pagamento: 'payments',
}

export default function DataImport({ lang = 'pt', locations = [], workers = [], setWorkers, setWorkDays, setPaymentRecords }) {
  ...
  const reasonText = (code, param) =>
    code === 'workerNotFound' ? t.reasonWorkerNotFound(param) : code === 'invalidDate' ? t.reasonInvalidDate : code === 'invalidCpf' ? t.reasonInvalidCpf(param) : code === 'duplicateName' ? t.reasonDuplicateName(param) : code === 'duplicateCpf' ? t.reasonDuplicateCpf(param) : t.reasonRequired(param)
    ...
    [
      {
        ...
        label: t.typeWorkers,
        icon: Users,
        n: byType.workers.length,
      },
      {
        key: 'workDays',
        label: t.typeWorkDays,
        icon: CalendarDays,
        n: byType.workDays.length,
      },
      {
        key: 'payments',
        label: t.typePayments,
        icon: DollarSign,
        n: byType.payments.length,
      },
    ].filter(e => e.n > 0)

  const handleDownloadTemplate = () => {
    const header = cfg.columns.map(c => c.header)
    const rows = type === 'all' ? ALL_EXAMPLES.map(ex => header.map(h => ex[h] ?? '')) : [cfg.columns.map(c => c.example)]
  ...
  const analyzeText = (text, fileName = '') => {
    ...
    if (type === 'all') {
      ...
      for (let r = 1; r < rows.length; r++) {
        ...
        if (!tp) {
          rejectedAll.push([...cols, t.reasonInvalidType])
          continue
        }
      ...
      for (const tp of ['diarista', 'dia', 'pagamento']) {
        ...
        for (const { cols, get, r } of groups[tp]) {
          ...
          if (missing.length) {
            rejectedAll.push([...cols, reasonText('required', missing.join(', '))])
            continue
          }
          const res = subCfg.build(get, { workers: liveWorkers, locations }, r)
          if (res.error) {
            rejectedAll.push([...cols, reasonText(res.error, res.param)])
            continue
          }
    ...
    for (let r = 1; r < rows.length; r++) {
      ...
      if (missing.length) {
        rejectedRows.push([...cols, reasonText('required', missing.join(', '))])
        continue
      }
      const res = cfg.build(get, { workers: liveWorkers, locations }, r)
      if (res.error) {
        rejectedRows.push([...cols, reasonText(res.error, res.param)])
        continue
      }
  ...
  const confirmImport = () => {
    ...
    log('import_csv', kind === 'all' ? `Importação CSV (tudo): ${byType.workers.length} diaristas, ${byType.workDays.length} dias, ${byType.payments.length} pagamentos, ${rejectedRows.length} rejeitada(s)` : `Importação CSV (${kind}): ${imported} ok, ${rejectedRows.length} rejeitada(s)`)
      ...
      <motion.div
        ...
        <AnimatePresence mode="wait">
          {!result && !preview ? (
            ...
            <motion.div key="preview" initial={{ opacity: 0, y: 8 }} animate={{ opacity: 1, y: 0 }} exit={{ opacity: 0 }} style={{ display: 'flex', flexDirection: 'column', gap: 16 }}>
              <div>
                <div
                  style={{
                    display: 'flex',
              ...
              <div
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 2 }}>
                  ...
                  <span
                    style={{
                      fontSize: 14,
                      fontWeight: result.imported > 0 ? 500 : 600,
                      color: result.imported > 0 ? 'var(--card-sub)' : 'var(--card-heading)',
                    }}>
