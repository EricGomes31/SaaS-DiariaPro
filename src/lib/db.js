...
function paymentFromRow(r) {
  return {
    ...
    weekdayEarnings: r.weekday_earnings,
    weekendEarnings: r.weekend_earnings,
    total: r.total,
    status: r.status,
...
function locationJobTitleToRow(j) {
  return {
    ...
    name: j.name,
    weekday_rate: j.weekdayRate,
    saturday_rate: j.saturdayRate,
    sunday_rate: j.sundayRate,
  }
}
...
function paymentToRow(p) {
  return {
    ...
    month_str: p.monthStr ?? null,
    total_days: p.totalDays ?? null,
    weekday_days: p.weekdayDays ?? null,
    weekend_days: p.weekendDays ?? null,
    weekday_earnings: p.weekdayEarnings ?? null,
    weekend_earnings: p.weekendEarnings ?? null,
    total: p.total ?? null,
...
// Utilitário de manutenção mantido de propósito (não usado no fluxo normal do app).
...
// Utilitário de seed mantido de propósito (não usado no fluxo normal do app).
async function _seedMockData() {
  await Promise.all([supabase.from('locations').insert(LOCATIONS.map(locationToRow)), supabase.from('holidays').insert(HOLIDAYS_2025.map(date => ({ date })))])
...
export async function fetchReportSubscribers() {
  const { data, error } = await supabase.from('report_subscribers').select('*').order('created_at')
  if (error) throw error
  return (data ?? []).map(r => ({
    id: r.id,
    name: r.name,
